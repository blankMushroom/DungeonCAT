...
public class Main extends JFrame implements MouseListener, KeyListener {
    ...
    public static void draw(Graphics2D g) {
        ...
        BufferedImage img = null;//new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
        try {
            img = ImageIO.read(new File("BrickWall.png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        //Рисуем кадр.

        g.drawImage(WFC.Transform(img), 0, 0, null);
    }
    public static void main(String[] args) throws InterruptedException {
        Main jf = new Main();
        jf.setSize(w, h);//размер экрана
        jf.setUndecorated(false);//показать заголовок окна
        jf.setTitle("TEST_DUNGEON");
        jf.setVisible(true);
